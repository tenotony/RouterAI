...
# ── 2. Download ────────────────────────────────
echo -e "  ${C}[2/4]${N} ดาวน์โหลด RouterAI..."
if [ -d "$DIR" ]; then
    echo -e "  ${Y}📁 พบโฟลเดอร์ $DIR — อัพเดต...${N}"
    cd "$DIR"
    # Backup config
...
# ── 4. Start ───────────────────────────────────
echo -e "  ${C}[4/4]${N} เริ่มระบบ..."
docker compose -f docker-compose.simple.yml down 2>/dev/null || true
docker compose -f docker-compose.simple.yml up -d --build 2>&1 | tail -5
